package com.djc.scdjc.http.cookies;
...
public class PersistentCookieStore {
    ...
    PersistentCookieStore() {
        cookiePrefs = App.getApplication().getSharedPreferences(COOKIE_PREFS, 0);
        cookies = new HashMap<>();
    ...
    private byte[] hexStringToByteArray(String hexString) {
        ...
        for (int i = 0; i < len; i += 2) {
            data[i / 2] = (byte) ((Character.digit(hexString.charAt(i), 16) << 4) + Character.digit(hexString
                    .charAt(i + 1), 16));
        }
        return data;
    }
